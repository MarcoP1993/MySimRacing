...
public class SalasActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        img_salas = findViewById(R.id.img_salas);
        btn_actualizar = findViewById(R.id.btn_act_salas);
        ...
        if(usuario !=null){
            if(usuario.getPhotoUrl() !=null){
                Picasso.get().load(usuario.getPhotoUrl()).into(img_salas);
            }
        }

        DocumentReference docRef = firestoredb.collection("Salas").document(idUsuario);
        docRef.addSnapshotListener(this, new EventListener<DocumentSnapshot>() {
            @Override
            public void onEvent(@Nullable DocumentSnapshot documentSnapshot , @Nullable FirebaseFirestoreException error) {

                StorageReference ref = reference.child("ImagenesSalas").child(idUsuario + ".jpg");
                ref.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                    @Override
                    public void onSuccess(Uri uri) {
                        Picasso.get().load(uri).into(img_salas);
                    }
                });
            }
        });
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == 100 && resultCode == Activity.RESULT_OK) {
            ...
            img_salas.setImageURI(imagenUrl);
    ...
    private void guardarImagenUsuario(Uri imagenUrl) {
        ...
        ref.putFile(imagenUrl).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
            ...
            public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                ...
                Toast.makeText(SalasActivity.this, "Imagen Guardada", Toast.LENGTH_SHORT).show();
            }
        }).addOnFailureListener(new OnFailureListener() {
            @Override
            public void onFailure(@NonNull Exception e) {
                Toast.makeText(SalasActivity.this, "Error al subir la foto", Toast.LENGTH_SHORT).show();
            }
        });
    }
